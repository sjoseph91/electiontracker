...
function LargestDonors(props){
    ...
    useEffect(() => {
        fetch(`${baseUrl}?api_key=${key}&committee_id=${committeeId}&two_year_transaction_period=2020&sort=-contribution_receipt_amount&sort_hide_null=true&is_individual=true&contributor_type=individual&cycle=2020&per_page=10`)
        .then(res =>res.json())
        .then(res => {
    ...
    const mappedResults = !donorData ? "loading" : donorData.map(donor => (
    ...
    return (
        <main className={`${candidate}-theme`}>
            <h3>Below are the largest invididual single donations by individuals for 2020.(multiple donations by the same individual were not counted)</h3>
            {mappedResults}
        </main>
    )
}
